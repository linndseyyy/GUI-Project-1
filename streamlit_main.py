...
if choice == 'Business Objective':  
    ...
    st.dataframe(comment_label_data[['ma_san_pham','noi_dung_binh_luan', 'label']].head(3))
    st.dataframe(comment_label_data[['ma_san_pham','noi_dung_binh_luan', 'label']].tail(3))  
...
# Add Product Search for Business Owner
    ...
    product_ids = customer_comment_data['ma_san_pham'].unique()
    selected_product = st.selectbox("Chọn mã sản phẩm", product_ids)
    # Lọc dữ liệu từ CSV1 theo Product ID đã chọn
    filtered_data = customer_comment_data[customer_comment_data['ma_san_pham'] == selected_product]
    ...
    if 'ngay_binh_luan' in customer_comment_data.columns:
        customer_comment_data['ngay_binh_luan'] = pd.to_datetime(customer_comment_data['ngay_binh_luan'], errors='coerce')
    ...
    if 'ngay_binh_luan' in customer_comment_data.columns:
        ...
        start_date = st.date_input(
            "Ngày bắt đầu", 
            min_value=customer_comment_data['ngay_binh_luan'].min(),
            max_value=customer_comment_data['ngay_binh_luan'].max()
        )
